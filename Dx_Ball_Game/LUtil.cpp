...
float x = 50, y = SCREEN_HEIGHT - 50 - PADDLE_HEIGHT - 25;
...
void welcomeDisplay()
{
    ...
    char msg1[] = "Dx Ball Game";
    char msg2[] = "Submitted by:";
    char msg3[] = "Anupom Roy - 1603102";
    char msg4[] = "Nibaron Kumar - 1603104";
    char msg6[] = "Aminul Hossain Sazzad - 1603105";
    ...
    glRasterPos3f(-270, 200, 0);
    for (int i = 0; i < strlen(msg4); i++)
    {
        glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, msg4[i]);
    }

    glRasterPos3f(-340, 250, 0);
...
void collisionDetection()
{
    ...
    if (y <= -180)
    {
        ...
        {
            for (j = 1; j < 6; j++)
                if (fabs(y - arrayX[0][j]) <= BALL_RADII && x < arrayX[i + 1][0] && x > arrayX[i][0])
                {
                    if (arrayX[i][j] != 0 )
                    {
                        score++;
    ...
    if (x >= right_wall - BALL_RADII)
    {
        h = -1;
    }
    //CHECK LEFT WALL IMPACT
    if (x <= left_wall + BALL_RADII)
    {
        h = 1;
    }

    //CHECK TOP WALL IMPACT
    if (y <= -(SCREEN_HEIGHT - 50 - BALL_RADII)) //0
    {
        v = 1;
    }
    //CHECK BOTTOME PADDLE IMPACT
    if ((x > paddle_left && x < paddle_right) && y >= SCREEN_HEIGHT - 50 - PADDLE_HEIGHT - BALL_RADII)
    {
...
void update()
{
    ...
    if (y > SCREEN_HEIGHT + BALL_RADII || score == 50) //condition when ball wont be within the reach of the paddle
    {
        printf("GAME OVER\n");
        printf("SCORE : %d\n", score);
        ...
        lak_ctr = 0;
        rak_ctr = 0;
        x = 50, y = SCREEN_HEIGHT - 50 - PADDLE_HEIGHT - 25;
